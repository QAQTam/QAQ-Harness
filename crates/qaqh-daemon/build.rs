...
fn ensure_webui_embed() {
    ...
    if !embedded.join("index.html").exists() && sibling.join("index.html").exists() {
        ...
        let _ = std::process::Command::new("cp")
            .args([
                ...
                embedded.to_string_lossy().as_ref(),
            ])
            .output();
    ...
    if !embedded.join("index.html").exists() {
        let _ = std::fs::create_dir_all(embedded);
        let placeholder = r#"<!doctype html><meta charset="utf-8"><title>QAQ Harness<><p style="font-family:system-ui;padding:2rem">前端产物缺失：请在 <code>qaqh-webui</code> 执行 <code>bun run build</code> 后重新 <code>cargo build -p qaqh-daemon</code>，或执行 <code>cp -r ../qaqh-webui/out/renderer crates/qaqh-daemon/webui-dist</code>。此占位由 build.rs 自动生成。</p>"#;
