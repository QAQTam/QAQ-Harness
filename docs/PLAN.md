...
> 状态：**待复核**（本计划由 codegraph 结构分析 + 4 个并行审查子代理产出，所有行号与场景结论均属"子代理已验证、主代理未二次复核"，执行前置 Step 0 强制复核）
...
| 指标 | 数值 |
|---|---|
| 发现总数 | **39**（🔴高 7 / 🟠中 20 / 🟢低 12） |
| 批次 | P0 核心 6 项 · P1 竞态/泄漏 21 项 · P2 清扫 12 项 |
...
| C2 | `loop_core.rs:1479-1560` | `ConversationCancel` 同时置 loop token 与全局 `set_cancel(true)`，但 UI 快捷 `ToolInvoke` 路径不清零 → 之后所有 UI 工具在 `execution.rs:63` 秒失败 | 🟠 | P1 |
...
*msgloop 干净区：PacedEmitter 即时发送不重排；round 编号一致；deferred_ringing FIFO 恰一次；serial 冲突序在权限挂起前后保持正确（approve/deny/mixed 排列已验证）。*
...
### 2.3 qaqh-message / msgloop 交界（存储层，10 项）
...
| G2 | `engine_compact.rs:158` ↔ `store.rs:1216` | `kept_user_count`(扁平 user 消息数，含 subagent 报告与旧摘要) 当 `keep`(Turn 结构数) 用 → 注入密集时 skip==0 早退但发 `CompactFinished{Completed}`（压缩静默停摆）；边界切半回合多折一 turn | 🟠 | P1 |
...
| G2 | store 新增按 turn 边界的 `compact_from_turn_index` API；engine 把 `msgs[kept_idx]` 经 msg_id 映射回所属 turn（排除 subagent 报告与 `[Compacted` 文本）；与 H4 同 PR 改压缩簇 | 注入密集会话压缩不再停摆 |
...
| 批 | 内容 |
|---|---|
| L-msgloop | ① YieldToUser 后 phase 复位 Idle ② plan/todo 死路径标注或删除 + 补 co-pending asks 陷阱注释 |
...
1. **Turn 身份守卫簇（H1+H2+M4+H5-B）**：抽单一 helper `validate_suspension(ctx, saved)` —— 校验 `session_id` + turn 未被取代 + store last step 归属 `saved.turn_id`；四个 resume 入口与新输入入口共用。
...
4. **压缩簇（H4+G2）**：store 侧一次性重构计数与“按 turn 边界压缩”API，两个调用方同步切换。
...
**完成标准**：39 项发现全部呈三态之一——已修 / 已否决(附理由) / 已立 issue；汇总到收尾报告并在本文档标记各项最终状态。
